...
app.post('/api/compile-theme', async (req, res) => {
	try {
		...
		// ui5.yaml + package.json so the export can be used directly as a UI5 theme-library project
		const ui5Yaml = `specVersion: "4.0"
metadata:
  name: themelib_${themeId}
type: theme-library
resources:
  configuration:
    paths:
      ...
      test: ""
`;
		await fs.writeFile(path.join(metaDir, 'ui5.yaml'), ui5Yaml);

		const packageJson = {
			name: `themelib_${themeId}`,
			version: "1.0.0",
			description: description || `Custom OpenUI5 theme "${themeName}" (based on ${baseTheme})`
		};
		await fs.writeFile(path.join(metaDir, 'package.json'), JSON.stringify(packageJson, null, 2));
...
`;
		...
		archive.file(path.join(metaDir, 'ui5.yaml'), { name: 'ui5.yaml' });
		archive.file(path.join(metaDir, 'package.json'), { name: 'package.json' });
